...
struct container    //structure for solving max man of the match
{
    int id;
    int id_count;
    char name[10];
...
struct container_1 //structure for solving max man of match 2
{
...
void man_of_the_match_array_create_1(struct team tm[],int size);
...
void print_sorted_output_1(struct container_1 p[],int size);
...
int main()
{
    ...
    man_of_the_match_array_create_1(tm,sz);
...
void insert_player_data(struct team tm[],int size)
{
    ...
    for(i=0; i<size; i++)
    {
...

        {
            ...
            fscanf(fp,"%d",&tm[i].p[j].previous_avg);

            //printf("no. of centuaries \n");
            fscanf(fp,"%d",&tm[i].p[j].century);

            //printf("player role\n");

            fscanf(fp," %c",&tm[i].p[j].player_role);

            //printf("player_name\n");

            fscanf(fp," %s",&tm[i].p[j].player_name);
...
void insert_data(struct match_played mp[],struct team tm[],int size_1,int size_2)
{
    ...
    for(i=0; i<size_1; i++)
    {
        ...
        for(k=0; k<NO_OF_PLAYING_TEAMS; k++)
        {
            ...
            //printf("TEAM%d_id\n",k+1);
            fscanf(fp,"%d",&mp[i].tp.playing_team_id[k]);
            ...
            for(j=0; j<TEAM_SIZE; j++)
            {
                ...
                fscanf(fp,"%d",&tm[a-1].p[j].present_match_score);

                // printf("present match wickets\n");
                fscanf(fp,"%d",&tm[a-1].p[j].present_match_wicket);
        ...
        //printf("HIGHEST_RUNS\n");
        fscanf(fp,"%d",&mp[i].highest_run);

        //printf("WICKET TAKEN BY PACERS\n");
        fscanf(fp,"%d",&mp[i].wickets_taken_by_pacer);

        // printf("MATCH RESULT\n");
        fscanf(fp," %c",&mp[i].match_result);

        //printf("MAN OF THE MATCH\n");
        fscanf(fp,"%d",&mp[i].man_of_the_match);
...
void man_of_the_match_array_create(struct match_played mp[],struct team tm[],int size)
{
    ...
    for(i=0; i<size; i++)
    {
        xarr[i]=man_of_match[i].id_count;
    }


    print_sorted_output(man_of_match,k);

    //man_of_the_match_array_create_1(tm,k);
...
void print_sorted_output(struct container p[],int size)
{
    printf("THE PLAYERS HAVING HIGHEST MAN OF THE MATCH ARE\n\n");
    ...
    while(flag!=0)
    {
        ...
        {
            printf("%d \n",p[maxpos].id);
            check_1[check_1_size]=p[maxpos].id;
            check_1_size++;
        }
        else
        {
...
void man_of_the_match_array_create_1(struct team tm[],int size)
{
    ...
    for(i=0; i<size; i++)
    {
        man_of_match[i].id_count=xarr[i];
    }
    ...
    for(i=0; i<size; i++)
    {
        if(man_of_match[i].id_count>=k)
        {
            q[index].id=man_of_match[i].id;
            q[index].century=tm[man_of_match[i].id/TEAM_SIZE].p[man_of_match[i].id%TEAM_SIZE].century;
            strcpy(q[index].name,man_of_match[i].name);

            index++;
        }

        // printf("%d\n",man_of_match[i].id);
        //printf("%d\n",man_of_match[i].id_count);
        //printf("%s\n",man_of_match[i].name);
    ...
    print_sorted_output_1(q,index);

}

void calculate_highest_individual_run_getter(struct match_played mp[],struct team tm[],int size_1,int size_2)
{
    printf("THE HIGHEST INDIVIDUAL RUN GETTERS ARE\n\n");
...
void print_sorted_output_1(struct container_1 p[],int size)
{
    ...
    maxcount=p[0].century;

    for(i=0; i<size; i++)
    {
        if(p[i].century>maxcount)
        {
            maxcount=p[i].century;
        }
    ...
    while(flag!=0)
    {
        ...
        if(p[maxpos].century==maxcount)
        {
            printf("%d \n",p[maxpos].id);
            check_2[check_2_size]=p[maxpos].id;
            check_2_size++;
        }
        else
        {
